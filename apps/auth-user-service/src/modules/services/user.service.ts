...
export class UsersService implements OnModuleInit{
  ...
  async findByEmail(email: string): Promise<User | null> {
    return await this.usersRepository.findOne({ where: { email }, relations: ['userRoles', 'userRoles.role'], });
  }
  ...
  async update(id: string, updateDto: UpdateUserDto) {
    ...
    // Extract roleIds from updateDto
    const { roleIds, password, ...restUpdateDto } = updateDto;
    ...
    // Sync roles if roleIds is provided
    if (roleIds !== undefined) {
      await this.userRoleService.syncUserRoles(id, roleIds);
    }
