...
export class AuthService {
  ...
  async login(user: any, loginDto: LoginDto) {
    ...
    const tokens = await this.generateTokens(payload.sub, payload.email, user.userRoles.map(ur => ur.role.code));
  ...
  async refreshToken(userId: string, rftoken: string) {
    ...
    const tokens = await this.generateTokens(userId, updateRt.user.email, updateRt.user.userRoles.map(ur => ur.role.code));
